...
parser.add_argument('--output_dir', type=str, default='your_project_directory_path/Automated_Extraction/models/CySecBert_crf/', 
                    help='Output directory for model')
parser.add_argument('--bert_model', type=str, default='markusbayer/CySecBERT', 
                    help='Pre-trained CySecBERT model to use')
...
class NerDataset(data.Dataset):
    ...
    def pad(cls, batch):
        ...
        predict_mask_list = torch.LongTensor(f(3, maxlen)).bool()
...
for epoch in range(start_epoch, total_train_epochs):
    ...
    if valid_f1 > valid_f1_prev:
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
                   ...
                   os.path.join(output_dir, 'CySecBert_crf_checkpoint.pt'))
